...
class UserBloc extends Bloc<UserEvent, UserState> {
  ...
  UserBloc({@required this.userRepository,@required this.userAuthenticationBloc}) : assert(userRepository !=null),
        super(userAuthenticationBloc.state is UserAuthenticationSuccess ?
      UserLoadSuccess(user: (userAuthenticationBloc.state as UserAuthenticationSuccess).user): UserLoadFailure() );
  ...
  Stream<UserState> _mapAddPictureRequestedToState(AddPictureRequested event) async* {
    yield UserLoading();
    try {
      await userRepository.putPicture(event.user, event.file);
      final user=await userRepository.getUser(event.user.username, event.user.token);
      yield UserLoadSuccess(user: user);
    } catch (_) {
      yield UserLoadFailure();
    }
